...
void Data::set_instrument_index(std::vector<size_t> index) {
  this->instrument_index = index;
  for (auto it = index.begin(); it != index.end(); ++it)
	  disallowed_split_variables.insert(*it);
}
...
size_t Data::get_number_instruments() const {
	return instrument_index.value().size();
}
...
Eigen::VectorXd Data::get_instrument(size_t row) const {
  Eigen::VectorXd instrument_values(instrument_index.value().size());
  for (size_t i = 0; i < instrument_index.value().size(); i++) {
	  instrument_values(i) = get(row, instrument_index.value().at(i));
  }
