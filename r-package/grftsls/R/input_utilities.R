...
validate_Z <- function(Z, X) {
  if (!is.matrix(Z)) {
    stop(paste("Z must be a matrix.")) 
  }
  
  if (!is.numeric(Z) && !is.logical(Z)) {
    stop(paste( "Observations (W or Y) must be numeric. GRF does not ",
                "currently support non-numeric observations."))
  }
  
  if (any(is.na(Z))) {
    stop("The vector of observations (W or Y) contains at least one NA.")
  }
  
  if (nrow(Z) != nrow(X)) {
    stop("length of observation (W or Y) does not equal nrow(X).")
  }
  Z
}
...
validate_observations <- function(V, X) {
  if (is.matrix(V) && ncol(V) == 1) {
    ...
    stop(paste( "Observations (W, Y) must be vectors."))
  }
