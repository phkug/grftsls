...
public:
  ...
  void set_instrument_index(std::vector<size_t> index);
  ...
  size_t get_number_instruments() const;

  double get_outcome(size_t row) const;
  double get_treatment(size_t row) const;
  Eigen::VectorXd get_instrument(size_t row) const;
...
protected:
  ...
  nonstd::optional<std::vector<size_t>> instrument_index;
